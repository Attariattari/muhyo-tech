        ...
        >
          ...
          <div className="relative h-full flex flex-col justify-center px-8 md:px-16 lg:px-24 max-w-4xl">
            ...
            >
              ...
              >
                Read Full Story <ArrowRight className="w-4 h-4 transition-transform group-hover/btn:translate-x-1" />
              </Link>
            </motion.div>
          </div>
        </motion.div>
        ...
        {featuredPosts.map((_, index) => (
          <button
            ...
            className={`h-1.5 rounded-full transition-all duration-500 ${
              index === currentIndex ? "w-12 bg-accent" : "w-3 bg-white/20 hover:bg-white/40"
            }`}
          />
        ))}
      </div>

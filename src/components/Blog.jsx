        ...
        <div className="absolute inset-0 p-10 md:p-16 flex flex-col justify-end space-y-6">
          ...
          <div className="flex items-center gap-8 pt-4 border-t border-white/10">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold uppercase tracking-widest text-white">{blog.author}</span>
            </div>
    ...
    <Link href={`/blog/${blog.id}`} className="block">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 p-6 rounded-[2rem] border border-border/10 bg-card/40 backdrop-blur-sm hover:border-accent/40 transition-all duration-500">
        ...
        <div className="md:col-span-3 flex flex-col justify-center space-y-4">
          <span className="text-accent text-[10px] font-black uppercase tracking-widest">{blog.category}</span>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-border/10 mt-auto">
          <div className="flex items-center gap-3">
             ...
             <div className="flex flex-col">
                <span className="text-[10px] font-bold uppercase tracking-widest text-foreground">{blog.author}</span>
                <span className="text-[8px] text-muted-foreground font-medium lowercase">Architect</span>
             </div>
          </div>
...
export default function Blog({ data }) {
  ...
  const regularGridPosts = filteredBlogs.filter(b => !searchQuery && activeCategory === "All" 
      ...
      <div className="max-w-7xl mx-auto px-6 mb-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
          <div className="space-y-6">
            ...
            >
              <Sparkles className="w-3.5 h-3.5" /> Insights & Editorial
            </motion.div>
            <h1 className="text-5xl md:text-8xl font-bold tracking-tighter leading-none">
              The <span className="text-accent">Perspective.</span>
            </h1>
            <p className="text-muted-foreground text-lg md:text-xl max-w-xl font-medium opacity-80 italic">
              Exploring the convergence of high-end software architecture and digital business growth.
            </p>
          </div>

          {/* Search Bar - Integrated into Header */}
          <div className="relative w-full md:w-[400px] h-fit">
            ...
            <input
              ...
              className="w-full bg-card/40 border border-border/10 rounded-[2rem] py-5 pl-16 pr-8 focus:outline-none focus:border-accent/40 focus:ring-4 focus:ring-accent/5 transition-all text-sm font-medium backdrop-blur-xl"
            />
          </div>
        </div>
        
        {/* Horizontal Category Scroller */}
        <div className="flex items-center gap-3 overflow-x-auto py-10 scrollbar-hide border-b border-border/10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
             ...
             <div className="lg:col-span-4 flex flex-col gap-10">
               ...
               <div className="flex-1 rounded-[2rem] bg-accent/5 border border-accent/20 p-8 flex flex-col justify-center items-center text-center space-y-4">
                  ...
                  <h4 className="text-xl font-bold tracking-tight">Stay Insightful</h4>
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-[0.2em] leading-relaxed">
                    Get my latest technical architecture deep-dives sent to your inbox.
                  </p>
      ...
      <div className="max-w-7xl mx-auto px-6 mb-16">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xs font-black uppercase tracking-[0.4em] text-foreground/60 leading-none">
            {isDiscoveryView ? "Latest from the Archive" : `Found Results (${filteredBlogs.length})`}
          </h2>
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {regularGridPosts.length > 0 ? (
            ...
            >
              ...
              <h3 className="text-3xl font-bold mb-4 tracking-tight">No Insights Found</h3>
              <p className="text-muted-foreground text-lg max-w-md mx-auto italic font-medium opacity-70">
                It seems we haven't covered this perspective yet. Try refining your keywords or exploring categories.
              </p>
            </motion.div>
          )}
        </AnimatePresence>

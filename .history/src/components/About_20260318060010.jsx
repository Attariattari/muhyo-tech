...
export default function About({ data, isHomePage = false }) {
  ...
  return (
    <div
      className={`relative overflow-hidden ${isHomePage ? " pt-0 pb-4 md:pb-8" : "pb-32"}`}
    >
      ...
      <SectionWrapper
        ...
        title="Muhyo Tech"
        className={isHomePage ? "sm:pb-0 pt-12 mb-0" : ""}
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start relative z-10">
          ...
          >
            ...
            <Tilt {...tiltOptions}>
              <div className="relative z-10 p-2 glass rounded-[2rem] overflow-hidden group border border-white/10 hover:border-accent/30 transition-all duration-700">
                <div className="relative w-full aspect-[4/5] rounded-[1.5rem] overflow-hidden shadow-2xl">
                  {/* Dynamic Overlays */}
          ...
          >
            ...
            >
              {data.values
                ?.slice(0, isHomePage ? 4 : undefined)
                .map((val, i) => (
                  <motion.div
                    key={i}
                    whileHover={{

...
export const SectionWrapper = ({ children, id, className = "", title, subtitle }) => {
  return (
    <section id={id} className={`py-24 px-6 md:px-12 lg:px-24 mx-auto max-w-7xl overflow-hidden ${className}`}>
      {title && (
        <motion.div
      ...
      {title && (
        <div className="text-center mb-16">
          <h4 className="text-accent text-xs font-black uppercase tracking-[0.3em] mb-4">{subtitle || "Overview"}</h4>
...
export const Card = ({ children, className = "" }) => (
  <div className={`glass p-8 rounded-2xl hover-glow transition-all ${className}`}>
    {children}
  </div>
);

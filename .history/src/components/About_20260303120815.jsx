    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        >
          <h3 className="mb-6">
            I am a Dedicated{" "}
            <span className="text-accent italic">Developer</span> Focusing on
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12">
            {data.values.map((val) => (
              <div
                key={val}
                className="flex items-center gap-4 text-foreground font-bold text-small tracking-widest uppercase"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {features.map((f, i) => (
              <div
                key={i}
                className="p-6 rounded-2xl glass border border-border/10 hover:border-accent/30 transition-all group"
              >
                <f.icon
                  size={24}
                  className="text-accent mb-4 group-hover:scale-110 transition-transform icon-scale"
                />
                <div className="font-bold text-small mb-1 uppercase tracking-widest">
                  {f.title}
                </div>
                <div className="text-xs text-secondary italic">{f.desc}</div>
              </div>
            ))}
          </div>
        </motion.div>

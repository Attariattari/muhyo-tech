...
import { Mail, Phone, MapPin, Send, Loader2, MessageSquare, ArrowRight } from "lucide-react";
          ...
          <div className="lg:col-span-5 space-y-12">
            ...
            >
              <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-foreground leading-[0.9] mb-6">
                Let's <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-blue-400">Collaborate</span> <br />
                on Your Vision.
              </h2>
              <p className="text-xl text-muted-foreground font-medium leading-relaxed">
                I'm currently available for freelance work and full-time opportunities. 
                If you have a project that needs a professional touch, let's talk.
              </p>
            </motion.div>
            ...
            <div className="space-y-6">
              {contactInfo.map((info, idx) => (
                ...
                >
                  <div className={`p-4 rounded-2xl ${info.color}/10 text-white ${info.color.replace('bg-', 'text-')} group-hover:scale-110 transition-transform duration-500 shadow-lg shadow-${info.color.split('-')[1]}/10`}>
                    {/* Fallback for icon color since dynamic text- classes can be tricky with Tailwind sometimes */}
                    <div style={{ color: info.label === 'Email' ? '#3b82f6' : info.label === 'Phone' ? '#10b981' : '#0ea5e9' }}>
                      {info.icon}
                    </div>
                  </div>
            ...
            <div className="p-8 glass rounded-[2.5rem] border border-accent/20 relative overflow-hidden">
               ...
               <h4 className="text-xl font-black text-foreground mb-2">Fast Response</h4>
               <p className="text-sm text-muted-foreground font-medium">
                 Expect to hear from me within 24 hours. I value your time and look forward to our synergy.
               </p>
            </div>
          </div>
            ...
            >
              <div className="bg-card/50 p-10 md:p-14 rounded-[3.2rem] border border-border/10 space-y-10 relative">
                <div className="space-y-2">
                   <h3 className="text-3xl font-black tracking-tight text-foreground">Send a Message</h3>
                   <p className="text-muted-foreground font-medium">Fill out the form below and I'll get back to you shortly.</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <label className={`text-[10px] font-black uppercase tracking-[0.2em] ml-2 transition-colors ${activeField === 'name' ? 'text-accent' : 'text-muted-foreground'}`}>
                        Your Name
                      </label>
                      <input
                    ...
                    <div className="space-y-3">
                      <label className={`text-[10px] font-black uppercase tracking-[0.2em] ml-2 transition-colors ${activeField === 'email' ? 'text-accent' : 'text-muted-foreground'}`}>
                        Email Address
                      </label>
                      <input
                  ...
                  <div className="space-y-3">
                    <label className={`text-[10px] font-black uppercase tracking-[0.2em] ml-2 transition-colors ${activeField === 'subject' ? 'text-accent' : 'text-muted-foreground'}`}>
                      Subject
                    </label>
                    <input
                  ...
                  <div className="space-y-3">
                    <label className={`text-[10px] font-black uppercase tracking-[0.2em] ml-2 transition-colors ${activeField === 'message' ? 'text-accent' : 'text-muted-foreground'}`}>
                      Message Details
                    </label>
                    <textarea

...
const StatBadge = ({ icon: Icon, text, subtitle, delay }) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay }}
      className="flex items-center gap-4 p-4 rounded-xl bg-background/40 backdrop-blur-md border border-white/10 hover:border-accent/40 hover:bg-background/60 transition-all group relative overflow-hidden"
    >
      <div className="absolute top-0 right-0 w-16 h-16 bg-accent/5 blur-2xl group-hover:bg-accent/10 transition-colors" />
      <div className="relative z-10 p-3 rounded-lg bg-accent/10 text-accent group-hover:scale-110 transition-transform duration-500">
        <Icon className="w-5 h-5" />
      </div>
      <div className="relative z-10">
        <div className="text-lg font-black text-foreground italic leading-tight">
          {text}
        </div>
        <div className="text-[9px] font-black uppercase tracking-[0.2em] text-accent/70">
          {subtitle}
        </div>
      </div>
      {/* Scanning Line Effect */}
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-accent/30 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-700" />
    </motion.div>
  );
};
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start relative z-10">
          ...
          >
            ...
            <div className="absolute -top-12 -left-12 text-[10rem] font-black text-accent/5 select-none pointer-events-none italic uppercase leading-none hidden lg:block">
              EXP
            </div>

            {/* Technical Frame Accents */}
            <div className="absolute -top-4 -left-4 w-12 h-12 border-t-2 border-l-2 border-accent z-20" />
            <div className="absolute -bottom-4 -right-4 w-12 h-12 border-b-2 border-r-2 border-accent z-20" />

            {/* Background Aura */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-accent/5 blur-[120px] rounded-full -z-10 animate-pulse" />

            <Tilt {...tiltOptions}>
              <div className="relative z-10 group cursor-crosshair">
                {/* Main Card Container */}
                <div className="relative p-2 bg-gradient-to-br from-white/10 to-transparent backdrop-blur-sm rounded-[2rem] border border-white/10 group-hover:border-accent/40 transition-all duration-700 overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.3)]">
                  {/* Decorative Industrial Pattern Overlay */}
                  <div className="absolute inset-0 opacity-[0.03] pattern-grid group-hover:opacity-[0.05] transition-opacity pointer-events-none" />
                  ...
                  <div className="relative w-full aspect-[4/5] rounded-[1.5rem] overflow-hidden">
                    <Image
                      ...
                      alt={data.name}
                      fill
                      className="object-cover transition-all duration-1000 group-hover:scale-105 group-hover:grayscale-[0.3]"
                      priority
                    />

                    {/* Multi-layered Overlays */}
                    <div className="absolute inset-0 bg-gradient-to-t from-background/95 via-background/20 to-transparent opacity-60 transition-opacity duration-700" />
                    <div className="absolute inset-0 bg-accent/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700 mix-blend-overlay" />

                    {/* Industrial Labeling */}
                    <div className="absolute top-8 right-8 writing-mode-vertical text-[10px] font-black uppercase tracking-[0.5em] text-accent/50 group-hover:text-accent transition-colors">
                      AUTH-ID: {data.name?.split(" ")[0]}
                    </div>

                    {/* Redesigned Premium ID Badge */}
                    <div className="absolute bottom-8 left-8 right-8 p-6 bg-background/60 backdrop-blur-2xl border border-white/10 rounded-2xl transform translate-y-4 group-hover:translate-y-0 transition-all duration-700 shadow-2xl overflow-hidden">
                      {/* Badge Top Decor */}
                      <div className="w-12 h-1 bg-accent/30 rounded-full mb-4" />

                      <div className="flex justify-between items-end">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-accent animate-ping" />
                            <div className="text-[9px] font-black uppercase tracking-[0.3em] text-accent italic">
                              Verified Status
                            </div>
                          </div>
                        ...
                        <div className="text-right">
                          <div className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/60 leading-none mb-1">
                            Designation
                          </div>
                          <div className="text-xs font-black text-accent uppercase italic">
                            Founder
                          </div>
                        </div>
                      </div>
                ...
                {/* Corner Label */}
                <div className="absolute -top-2 -right-2 px-3 py-1 bg-accent text-accent-foreground text-[9px] font-black italic uppercase tracking-widest rounded-sm z-30 shadow-lg shadow-accent/20">
                  EST. 2021
                </div>
              </div>
            </Tilt>

            {/* Redesigned Stats Console */}
            <div className="absolute -bottom-12 right-0 md:-right-8 lg:-right-16 z-20 flex flex-col gap-4 w-fit min-w-[220px]">
              <StatBadge
                icon={Sparkles}
                text="3+ Years"

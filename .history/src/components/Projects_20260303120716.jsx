    ...
    <Card className="p-4 border border-border/10 overflow-hidden group">
      <div className="relative aspect-video rounded-xl bg-card/60 overflow-hidden border border-border/10 mb-6 group-hover:shadow-2xl transition-all duration-300">
        <div className="absolute inset-0 bg-accent/20 opacity-0 group-hover:opacity-100 transition-opacity z-10 flex items-center justify-center gap-4">
          <button className="p-4 rounded-full bg-accent text-accent-foreground hover:scale-110 transition-transform shadow-lg">
            <Link2 size={20} />
          </button>
          <button className="p-4 rounded-full bg-slate-900 text-white hover:scale-110 transition-transform shadow-lg">
            <Github size={20} />
          </button>
        </div>
        <img
        ...
        <div className="absolute top-4 left-4 z-20">
          <span className="px-3 py-1 rounded-full bg-[#0a0f1c]/80 backdrop-blur text-xs font-black uppercase text-accent tracking-widest border border-accent/30">
            {project.purpose}
          </span>
        </div>
      </div>
      ...
      <div className="px-4 pb-4">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {project.techStack.map((tech) => (
            <span
              key={tech}
              className="text-xs font-bold px-3 py-1 rounded-full bg-muted/50 border border-border text-foreground tracking-widest uppercase transition-all hover:bg-border/50"
            >
              {tech}
            </span>
          ))}

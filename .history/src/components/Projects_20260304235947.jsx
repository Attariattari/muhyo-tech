    ...
    <Card className="p-4 border border-border/10 overflow-hidden group">
      <div className="relative aspect-video rounded-xl bg-card/60 overflow-hidden border border-border/10 mb-6 group-hover:shadow-2xl transition-all duration-300">
        ...
        <div className="absolute top-4 left-4 z-20">
          <span className="px-3 py-1 rounded-full bg-[#0a0f1c]/80 backdrop-blur text-[9px] font-black uppercase text-accent tracking-widest border border-accent/30">
            {project.purpose}
          </span>
        </div>
      </div>
      ...
      <div className="px-4 pb-4">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {project.techStack.map((tech) => (
            <span
              key={tech}
              className="text-[9px] font-bold px-2 py-0.5 rounded bg-muted/50 border border-border text-foreground tracking-widest uppercase"
            >
              {tech}
            </span>
          ))}
        ...
        <button className="w-full py-3 rounded-lg border border-accent/20 bg-accent/5 hover:bg-accent hover:text-accent-foreground text-xs font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 group">
          See Project Details <ExternalLink className="w-4 h-4" />
        </button>
      </div>
    </Card>
  </motion.div>

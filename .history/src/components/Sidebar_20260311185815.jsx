...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Glass Header */}
      <div className="lg:hidden fixed top-0 left-0 w-full h-20 bg-background/80 backdrop-blur-xl border-b border-border/50 z-[60] px-8 flex items-center justify-between">
        <Link href="/" className="group flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-accent flex items-center justify-center overflow-hidden shadow-lg shadow-accent/20">
            <img
              src="/logo.png"
              alt="Muhyo Tech"
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="w-12 h-12 rounded-2xl bg-muted/30 border border-border flex items-center justify-center text-accent hover:bg-accent/10 transition-all"
          >
      ...
      <aside
        className={`fixed left-4 top-4 bottom-4 glass rounded-[2.5rem] z-50 hidden lg:flex flex-col transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] border border-border/50 shadow-2xl group overflow-hidden ${
          isCollapsed ? "w-20" : "w-64"
        }`}
      >
        <div className="flex flex-col h-full overflow-y-auto overflow-x-hidden custom-scrollbar relative">
        ...
        <button
          ...
          className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-20 glass border border-border rounded-full flex items-center justify-center text-muted-foreground hover:text-accent transition-all hover:h-24 group/toggle shadow-xl z-50"
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden fixed inset-0 bg-background z-[70] p-10 flex flex-col overflow-y-auto custom-scrollbar"
          >
            <div className="flex justify-between items-center mb-16">
              <span className="text-3xl font-black text-foreground italic tracking-tighter uppercase">
                Muhyo
              ...
              <button
                ...
                className="w-14 h-14 rounded-full bg-card border border-border flex items-center justify-center text-foreground"
              >
                <X size={32} />
              </button>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <GripVertical size={20} className="text-foreground/10" />
                    </Link>
                  </motion.div>
                );
              })}
            ...
            <div className="mt-auto pt-10 border-t border-border flex flex-col gap-6">
              <div className="flex items-center justify-between p-6 glass rounded-3xl border border-border">
              ...
              <Link
                href="/resume"
                className="w-full h-20 flex items-center justify-center gap-4 rounded-3xl bg-accent text-accent-foreground font-black uppercase tracking-[0.4em] text-sm shadow-2xl shadow-accent/20"
                onClick={() => setIsOpen(false)}
              >
                <FileUser size={24} />
                My Resume
